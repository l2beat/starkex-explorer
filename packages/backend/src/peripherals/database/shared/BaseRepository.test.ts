...
describe(BaseRepository.name, () => {
  ...
  describe(BaseRepository.prototype.autoWrap.name, () => {
    ...
    describe.skip('checkConvention', () => {
      ...
      describe('AddMethod', () => {
        it('should not show error if convention is correct', () => {
          class DummyRepository extends BaseRepository {
            constructor(database: Database, logger: Logger) {
              super(database, logger)
              this.autoWrap<CheckConvention<DummyRepository>>(this)
            ...
            async addWithNumberAsReturnType(a: string): Promise<number> {
              return 1
            }

            async addWithStringAsReturnType(a: string): Promise<string> {
              return '1'
            }
          }
        })
