...
describe(ForcedTradeOfferRepository.name, () => {
  ...
  describe(
    ForcedTradeOfferRepository.prototype.getFinalizableByStarkKey.name,
    () => {
      it('returns the offers that can be finalized by the user', async () => {
        const starkKey = StarkKey.fake()
        const makerOffer = fakeOffer({
          starkKeyA: starkKey,
          accepted: undefined,
        })
        const acceptedOffer = fakeOffer({
          starkKeyA: starkKey,
          accepted: fakeAccepted(),
        })
        const expiredOffer = fakeOffer({
          starkKeyA: starkKey,
          accepted: {
            ...fakeAccepted(),
            submissionExpirationTime: Timestamp(new Date().getTime() - 1000),
          },
        })
        const cancelledOffer = fakeOffer({
          starkKeyA: starkKey,
          cancelledAt: Timestamp(1),
        })
        const finalizedOffer = fakeOffer({
          starkKeyA: starkKey,
          accepted: {
            ...fakeAccepted(),
            transactionHash: Hash256.fake(),
        ...
        const takerOffer = fakeOffer({
          accepted: fakeAccepted({ starkKeyB: starkKey }),
        })
        const notInvolvedOffer = fakeOffer()

        await repository.add(makerOffer)
        await repository.add(acceptedOffer)
        await repository.add(expiredOffer)
        await repository.add(cancelledOffer)
        await repository.add(finalizedOffer)
        await repository.add(takerOffer)
        await repository.add(notInvolvedOffer)

        expect(
          await repository.getFinalizableByStarkKey(starkKey)
        ).toEqualUnsorted([acceptedOffer])
      })
    }
  )
