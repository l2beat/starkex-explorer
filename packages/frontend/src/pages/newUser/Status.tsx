...
export function Status({status}: StatusProps) {
    return(
        <div className={cx('px-2 py-1 rounded-full font-bold text-xs w-max', {'text-white': status === 'EXPIRED' || status === 'CANCELLED' || status === 'REVERTED'}, {'text-background': status !== 'EXPIRED' && status !== 'CANCELLED' && status !== 'REVERTED'}, backgroundMap[status])}>
            {status}
        </div>
    );
}
