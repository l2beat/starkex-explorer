...
export function EthereumTransactionStatus({status}: EthereumTransactionStatusProps) {
    return (
        <div className={cx('px-2 py-1 rounded-full font-bold text-xs w-max', {'text-background': status !== 'REVERTED'}, {'text-white': status === 'REVERTED'}, backgroundMap[status])}>
            {status}
        </div>
    )
}